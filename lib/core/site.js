...
class Site {
    ...
    async updateList(id, cmd, isTemp, pauseTimer) {
        ...
        switch (cmd) {
            ...
            case UpdateCmd.REMOVE:
                ...
                break;
            default:
                this.print(dvr_js_1.MSG.ERROR, "Unexpected cmd type");
                break;
        }
        return dirty;
    }
    ...
    async checkStreamerState(streamer, options) {
        ...
        if (this.dvr.tryingToExit) {
            this.print(dvr_js_1.MSG.INFO, `${colors.name(streamer.nm)} skipping lookup due to shutdown request`);
            return;
        }
    ...
    async getStreamers() {
        ...
        this.print(dvr_js_1.MSG.DEBUG, "processing streamers");
        await this.processStreamers();
        this.print(dvr_js_1.MSG.DEBUG, "done processing streamers");
        return true;
    }
    ...
    haltCapture(uid) {
        if (this.streamerList.has(uid)) {
            ...
            if (streamer && streamer.capture !== undefined && streamer.postProcess === false) {
                streamer.capture.kill("SIGINT");
                this.print(dvr_js_1.MSG.INFO, `${colors.name(streamer.nm)} recording stopped`);
            }
        }
    }
    async writeConfig() {
