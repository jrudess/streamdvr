...
class Site {
    ...
    async processUpdates(cmd, isTemp, pauseTimer) {
    ...
    async updateStreamers(list, cmd, isTemp, pauseTimer) {
        let dirty = false;
    ...
    addStreamer(id, list, cmd, isTemp) {
        let added = true;
        for (const entry of list) {
            if (entry[0] === id.uid) {
        ...
        if (!this.streamerList.has(id.uid)) {
            const newstreamer = exports.StreamerDefaults;
            newstreamer.uid = id.uid;
            newstreamer.nm = id.nm;
            newstreamer.site = this.padName;
            newstreamer.isTemp = isTemp ? true : false;
            newstreamer.paused = this.paused;
            this.streamerList.set(id.uid, newstreamer);
            this.render(true);
            this.refresh(newstreamer);
        }
        return added;
    }
    ...
    refresh(streamer) {
        if (!this.dvr.tryingToExit && this.streamerList.has(streamer.uid)) {
            const options = exports.StreamerStateDefaults;
    ...
    endCapture(streamer, capInfo) {
        ...
        try {
            const stats = fs.statSync(this.dvr.config.recording.captureDirectory + "/" + fullname);
            if (stats) {
