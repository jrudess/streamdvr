...
class PostProcess {
    ...
    async convert() {
        ...
        if (fileType === "ts") {
            this.dvr.print(dvr_1.MSG.DEBUG, `${namePrint} moving ${capPath} to ${cmpPath}`);
        ...
        this.dvr.print(dvr_1.MSG.INFO, `${namePrint} converting recording to ${fileType}`, site);
        this.dvr.print(dvr_1.MSG.DEBUG, `${namePrint} ${colors.cmd(script)} ${colors.cmd(args.join(" "))}`, site);
        const myCompleteProcess = (0, child_process_1.spawn)(script, args);
        if (site && streamer) {
            ...
            void new Promise(async () => {
                if (!this.config.recording.keepTsFile) {
                    try {
                        ...
                        this.dvr.print(dvr_1.MSG.ERROR, `${args[0]} does not exist, cannot remove`);
                    }
                }
                this.dvr.print(dvr_1.MSG.INFO, `${namePrint} done converting ${colors.file(completeFile)}`, site);
                await this.postScript(site, streamer, completeDir, completeFile);
            });
        });
    ...
    async postScript(site, streamer, completeDir, completeFile) {
        ...
        this.dvr.print(dvr_1.MSG.DEBUG, `${namePrint} running global postprocess script: ` +
            `${colors.cmd(script)} ${colors.cmd(args.join(" "))}`, site);
        const userPostProcess = (0, child_process_1.spawn)(script, args);
        if (site && streamer) {
            site.storeCapInfo(streamer, completeFile, userPostProcess, true);
        }
        userPostProcess.on("close", () => {
            this.dvr.print(dvr_1.MSG.INFO, `${namePrint} done post-processing ${colors.file(completeFile)}`, site);
            void new Promise(async () => {
                await this.nextConvert(site, streamer);
            });
        });
    ...
    async uniqueFileName(completeDir, filename, fileType) {
        ...
        try {
            while (await fsp.access(name, fsp.F_OK)) {
                this.dvr.print(dvr_1.MSG.ERROR, name + " already exists");
            ...
            if (err) {
                if (err.code === "EXDEV") {
                    try {
                        ...
                        if (err) {
                            this.dvr.print(dvr_1.MSG.ERROR, `${colors.site(oldPath)}: ${err.toString()}`);
                        }
                    }
                }
                else {
