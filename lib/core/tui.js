            ...
            switch (index) {
                case 0: // pause
                    if (this.listSelect && this.listSelect.length >= 2) {
                        ...
                        void this.updateStreamerList(site, name, site_1.UpdateCmd.PAUSE);
                        this.listmenu.hide();
                        this.list.focus();
                        this.render(false);
                    }
                ...
                case 2: // remove
                    if (this.listSelect && this.listSelect.length >= 2) {
                        const site = blessed.helpers.stripTags(this.listSelect[2]).trim();
                        const name = blessed.helpers.stripTags(this.listSelect[0]).trim();
                        void this.updateStreamerList(site, name, site_1.UpdateCmd.REMOVE);
                        this.listmenu.hide();
                        this.list.focus();
                        this.render(false);
                    }
        ...
        this.inputBar.on("submit", (text) => {
            ...
            if (this.list.interactive) {
                ...
                if (this.sitelistSelect) {
                    const site = blessed.helpers.stripTags(this.sitelistSelect[0]).trim();
                    void this.updateStreamerList(site, text, site_1.UpdateCmd.ADD);
                }
                this.sitemenu.focus();
                this.render(false);
                return;
