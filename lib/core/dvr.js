...
class Dvr {
    constructor(dir) {
        ...
        if (this.config.tui.enable) {
            this.tui = new tui_1.default(this);
        }
        this.postProcess = new postprocess_1.default(this);
    ...
    run(site) {
        return __awaiter(this, void 0, void 0, function* () {
            let startup = true;
            yield site.getStreamers({ init: startup });
            while (true) {
                try {
                    if (!startup) {
                        yield site.disconnect();
                        yield site.connect();
                    }
                    yield site.processUpdates({ add: true, init: startup });
                    yield site.processUpdates({ add: false, init: startup });
                    yield site.getStreamers();
                    startup = false;
                }
                catch (err) {
