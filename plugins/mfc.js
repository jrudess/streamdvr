...
const {Site} = require(process.cwd() + "/core/site");
...
class Mfc extends Site {
    constructor(name, dvr, tui, urlback) {
        ...
        if (typeof this.config.mfcautolog !== "undefined" && this.config.mfcautolog === false) {
            mfc.setLogLevel(0);
        }
        this.mfcGuest = new mfc.Client("guest", "guest", {
            useWebSockets: this.config.mfcWebSocket,
            modernLogin:   this.config.modernLogin,
            camYou:        false
        });
        ...
        if (this.mfcGuest.state === mfc.ClientState.ACTIVE) {
            try {
                ...
                if (id) {
                    ...
                    this.errMsg(nm.name + " does not exist on this site");
                }
            } catch (err) {
                this.errMsg(err.toString());
            }
    ...
    async checkStreamerState(uid, options) {
        ...
        if (!this.streamerList.has(uid)) {
            this.streamerList.set(uid, {uid: uid, nm: model.nm, site: this.padName, state: "Offline", filename: "", capture: null, postProcess: 0});
        ...
        if (bestSession.vs === mfc.STATE.FreeChat) {
            ...
            msg += msg.charAt(msg.length - 6) === "s" ? "'".name : "'s".name;
            msg += " stream is off.";
    ...
    async getStreamers(options) {
        ...
        for (let i = 0; i < this.config.streamers.length; i++) {
            queries.push(this.checkStreamerState(this.config.streamers[i], options));
        }

        // Only add a streamer from temp list if they are not
        // in the primary list.  Prevents duplicate recording.
        for (let i = 0; i < this.tempList.length; i++) {
            if (!_.contains(this.config.streamers, this.tempList[i])) {
    ...
    setupCapture(model) {
        ...
        if (mod.camserv < 840) {
            this.infoMsg(model.nm.name + " does not have a mobile feed and can not be captured");
            return {spawnArgs: "", filename: "", streamer: ""};
        }
        ...
        const params = [];
        if (this.config.recorder === "scripts/record_streamlink.sh") {
